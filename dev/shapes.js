...
// six circles (circleWeights, the same function the matcher uses) and whole-cell coverage, picks a
// density ramp from the measured coverage, and saves the module text to shots/shape-vectors.js.
import { ASCII_CHARSET, CELL_ASPECT, CIRCLES, CIRCLE_R, GEOMETRY_KEY, SHIFTS, circleWeights } from '../js/ascii.js';
...
// Normalise each dimension by its maximum over the set (Harri): the densest glyph per circle is 1.
const dimMax = [0, 1, 2, 3, 4, 5].map(k => Math.max(...rawVec.map(v => v[k])));
const vectors = shifted.map(list => list.map(v => v.map((x, k) => x / dimMax[k])));
...
lines.push('// VECTORS: 6 numbers per glyph = mean ink under the six circles of js/ascii.js (TL, TR, ML, MR,');
lines.push('// BL, BR), each dimension divided by its maximum over the unshifted set; one row per SHIFTS entry');
lines.push('// (glyph drawn that fraction of a cell sideways). COVERAGE: ink fraction of the cell.');
...
glyphs.forEach((ch, i) => {
  ...
  CIRCLES.forEach(([cx, cy], k) => {
    ...
    s.fillStyle = `rgba(220, 60, 40, ${0.55 * vectors[i][0][k]})`;
    s.fill();
